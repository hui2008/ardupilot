...
H-bridge. The board uses `RCOutput_Duffy` for motor output, ArduPilot's serial
RC protocol path for receiver input, and an MPU6500 IMU on Raspberry Pi SPI0
CE0 (`/dev/spidev0.0`).

The normal bring-up path is:

1. Verify the PCA9685/L298N wiring.
2. Verify the MPU6500 is visible on SPI.
3. Verify which Linux UART backs `/dev/serial0`.
4. Remove Linux serial console ownership from the receiver UART.
5. Confirm Linux can see I.Bus bytes from the receiver.
6. Start Rover with `SERIAL0` on TCP and `SERIAL1` on `/dev/serial0`.
7. Calibrate the MPU6500 and confirm parameter storage persists.
...
The rover output defaults map ArduPilot Rover's skid-steer functions to the two
logical motor outputs:

```text
SERVO1_FUNCTION 73  # ThrottleLeft, HAL output channel 0
SERVO3_FUNCTION 74  # ThrottleRight, HAL output channel 2
```

### IMU

The board hardware definition enables an MPU6500 on Raspberry Pi SPI0 CE0:

```text
IMU Invensense SPI:mpu6500 ROTATION_NONE
LINUX_SPIDEV "mpu6500"  0   0   SPI_MODE_0   8   SPI_CS_KERNEL   1*MHZ   11*MHZ
```

On Linux this is expected to appear as:

```sh
ls /dev/spidev0.0
```

A direct SPI probe should report the MPU6500 `WHO_AM_I` value, normally
`0x70`, before debugging ArduPilot sensor startup.
...
Enable SPI and the Raspberry Pi UART in the OS boot configuration. On newer
Raspberry Pi OS images this is normally in `/boot/firmware/config.txt`; on
older images it may be in `/boot/config.txt`.
...
### Remove Linux Console Ownership
...
There should be no serial console entry for the receiver UART, such as:
...
If one is present, remove it from `cmdline.txt`. On newer Raspberry Pi OS
images the boot command line is normally `/boot/firmware/cmdline.txt`; on older
images it may be `/boot/cmdline.txt`. Keep `cmdline.txt` as a single line.
...
After reboot, verify that nothing has the UART open before starting Rover:
...
`libraries/AP_HAL_Linux/hwdef/duffy/hwdef.dat`:
...
distance from `1500 us` determines the PCA9685 enable-channel duty cycle. Zero
is preserved as a stop command instead of being clamped to `1000 us`; clamping
zero would command full reverse during safety or shutdown paths.
...
ArduPilot-side RC debug points:
